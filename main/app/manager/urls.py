...
class UrlBase(object):
    ...
    def formatUrl(cls, string):
        ...
        base_name, url_id = cls.splitBaseId(string)
        return cls.universal().get_url(base_name) % url_id
...
class UrlManager(UrlBase):
    ...
    def setTitleIndex(self, title):
        ...
        if count > 0:
            ...
            try:
                index = int(matchobj.group("index"))
            except:
                index = 0
            title += "###%d" % (index + 1)
        return title
    ...
    def saveLast(self, url, title):
        try:
            lasturl = self.models.LastUrl.objects.latest("url")
        except:
            lasturl = self.models.LastUrl()
    ...
    def getUrlTitle(self, url):
        try:
            query = self.objects.get(_url=self.shortUrl(url))
        except:
            query = self.models.Url(title="")
        return query.title
    ...
    def getLastUrl(self):
        ...
        try:
            query = self.models.LastUrl.objects.latest("url")
        except:
            query = self.models.LastUrl(url="http://", title="...")
