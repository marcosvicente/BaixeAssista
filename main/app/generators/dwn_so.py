...
class DwShare(SiteBase):
    ...
    urls = ("http://st.dwn.so/xml/videolink.php?v=%s", "http://dwn.so/xml/videolink.php?v=%s")
    ...
    def start_extraction(self, proxies={}, timeout=25):
        ...
        for url in self.urls:
            try:
                request = self.connect(url % video_id, proxies=proxies, timeout=timeout)
        ...
        url = re.search("un=(?:'|\")(.*?)(?:'|\")", xml_data, re.DOTALL).group(1)
        ...
        try:
            title = re.search("title\s*=\s*(?:'|\")(.*?)(?:'|\")", xml_data, re.DOTALL).group(1)
        except:
